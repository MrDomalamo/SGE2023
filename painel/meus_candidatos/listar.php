...
if($nivel_usuario == 'Diretor' || $nivel_usuario == 'Recrutador'){
	$query = $pdo->query("SELECT * FROM candidatos where bairro = '$provincia_usu'  ORDER BY id desc");
}else{
...
if($total_reg > 0){
	...
	<th class="esc">BI / Passaporte</th> 
	<th class="esc">Email</th>
	<th class="esc">Instituicão</th>	 
	<th>Ações</th>
	</tr> 
	</thead> 
	<tbody> 
HTML;
	for($i=0; $i < $total_reg; $i++){
		...
		<big><a href="#" onclick="mostrar('{$nome}', '{$telefone}', '{$cpf}', '{$email}', '{$endereco}', '{$nome_cargo}', '{$data_nascF}','{$data_cadF}', '{$idade}', '{$nacionalidade}', '{$foto}', '{$genero}', '{$nome_cidade}', '{$nome_bairro}', '{$nome_curso}', '{$nome_instituicoes}', '{$estado_civil}', '{$nome_nivel_academico}')" title="Ver Dados"><i class="fa fa-info-circle text-secondary"></i></a></big>
...
<script type="text/javascript">
	...
	function editar(id, nome, telefone, cpf, email, endereco, cargo, idade, data_nasc ,nacionalidade, foto,genero, cidade, bairro, instituicoes,curso, estado_civil, nivel_academico){
