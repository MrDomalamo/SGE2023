...
<body>	
	...
	<?php } ?>
	<div class="edm" ><B>Relatório Candidaturas Concluidas</B></div>
	<br>
	...
	<div class="mx-2" style="padding-top:10px; ">
		...
		<?php 
			...
			if(($genero == "") && ($idade == "")){
				$query = $pdo->query("SELECT * FROM candidaturas where estado LIKE 'Concluido' and (data_cad >= '$dataInicial' and data_cad <= '$dataFinal')  ORDER BY id desc");
			}elseif(($genero == "Masculino" || $genero == "Feminino") && ($idade == "")){
				$query = $pdo->query("SELECT * FROM candidaturas where estado LIKE 'Concluido' and (data_cad >= '$dataInicial' and data_cad <= '$dataFinal') and genero LIKE '$genero'  ORDER BY id desc");
			}elseif(($genero == "") && ($idade >= 18)){
				$query = $pdo->query("SELECT * FROM candidaturas where estado LIKE 'Concluido' and (data_cad >= '$dataInicial' and data_cad <= '$dataFinal') and idade LIKE '$idade'  ORDER BY id desc");
			}elseif(($genero == "Masculino" || $genero == "Feminino") && ($idade >= 18)){
				$query = $pdo->query("SELECT * FROM candidaturas where estado LIKE 'Concluido' and (data_cad >= '$dataInicial' and data_cad <= '$dataFinal') and idade LIKE '$idade' and genero LIKE '$genero'  ORDER BY id desc");
			}
			...
			<small><small>
				<section class="area-tab" style="background-color: #D2691E;">
					...
					<div class="linha-cab" style="padding-top: 5px; color: #000;">
						...
						<div class="coluna" style="width:15%; margin-left: 105px" ><b>Instituição</b></div>
						<div class="coluna" style="width:12%; margin-left: -30px" ><b>Cidade</b></div>
		...
		$data_cadF = implode('/', array_reverse(explode('-', $data_cad)));
					...
					<section class="area-tab" style="padding-top:10px; padding-bottom: 10px;">					
						<div class="linha-cab">
							...
							<div class="coluna" style="width:30%" ><?php echo $nome_curso ?></div>
