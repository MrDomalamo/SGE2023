...
echo <<<HTML
			<tr class="{$classe_linha}"> 
				...
				<td class="{$classe}"> {$valor} MNZ</td>		
				<td class="{$classe_saldo_periodo}"> {$total_saldo_periodoF} MNZ</td>
...
echo <<<HTML
	...
	<div align="right" style="margin-top:15px; background:white; padding-top:10px">
		Saldo do Período: <span class="{$classe_saldo}"> {$total_saldoF} MNZ</span><br>
		</div>
</small>
HTML;
}else{
...
<script type="text/javascript">
	...
	$(document).ready( function () {
	    ...
	    $('#tabela_filter label input').focus();
			...
			$('#total_itens').text(' <?=$total_saldo_geralF?> MNZ');
	} );
