...
//retirar aspas do texto do obs
		...
		//RECUPERAR O TEXTO DA VISTORIA CASO EXISTA
		$query2 = $pdo->query("SELECT * FROM contrato_aluguel where imovel = '$id'");
		$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
		if(@count($res2) > 0){
			$texto_contrato = $res2[0]['texto'];			
		}else{
			$texto_contrato = '';	
		}
		...
		//retirar aspas do texto do obs
		$texto_contrato = str_replace('"', "**", $texto_contrato);
		$texto_contrato = str_replace(array("\n", "\r"), ' + ', $texto_contrato);
		...
		<a href="#" onclick="contrato('{$id}', '{$titulo}', '{$texto_contrato}')" title="Gerar Contrato"><i class="fa fa-file-archive-o " style="color:blue; margin-left:3px"></i></a>
...
function contrato(id, nome, texto){
    ...
    $('#id-aluguel').val(id);        
    $('#nome-aluguel').text(nome);
    nicEditors.findEditor("area2").setContent(texto);	
    $('#modalContratoAluguel').modal('show');
    $('#mensagem-aluguel').text(''); 
