...
if($nivel_usuario == 'Recrutador' || $nivel_usuario == 'Diretor'){
	$query = $pdo->query("SELECT * FROM candidaturas where ativo = 'Não' and bairro = '$provincia_usu' ORDER BY id desc");
}else{
	$query = $pdo->query("SELECT * FROM candidaturas where ativo = 'Não' ORDER BY id desc");
}

// if($nivel_usuario == 'Candidato'){
// 	$query = $pdo->query("SELECT * FROM candidaturas where ativo = 'Não' and candidato = '$id_usuario' ORDER BY id desc");
// }else{
// 	$query = $pdo->query("SELECT * FROM candidaturas where ativo = 'Não' ORDER BY id desc");
// }
$res = $query->fetchAll(PDO::FETCH_ASSOC);
...
if($total_reg > 0){
	...
	<tr>  
	<th>Nome</th>
	<th class="esc">Instituicao</th> 
	<th class="esc">Curso</th> 
	<th class="esc">Genero</th>
	<th class="esc">Telefone</th>	  
	<th class="esc">Estagio</th>
	<th class="esc">Obs</th>
	<th>Ações</th>
	</tr> 
	</thead> 
...
if($ext == 'pdf'){
	...
	$tumb_arquivo = $arquivo;
		...
		<big><a href="#" onclick="mostrar('{$nome_candidato}', '{$telefone_candidato}', '{$genero_candidato}', '{$nome_curso}', '{$nome_instituicoes}' , '{$nome_nivel_academico}', '{$finalidade}' ,'{$nome_bairro}' ,'{$nome_cidade}'  ,'{$nome_direcoes}' ,'{$nome_pelouro}' , '{$data_cadF}', '{$data_inicioF}','{$data_finalF}' ,'{$estado}', '{$descricao}')" title="Ver Dados"><i class="fa fa-info-circle text-secondary"></i></a></big>
...
<script type="text/javascript">
	...
	function mostrar(nome, telefone, genero, curso, instituicoes, nivel_academico, finalidade, bairro, cidade, direcoes, pelouro, data_cad, data_inicio, data_final, estado, descricao){
		...
		// $('#target_mostrar').attr('src','images/arquivos/{$tumb_arquivo}');		
