...
if(@count($res2) > 0){
	$id_diretor = $res2[0]['id_func'];
}else{

	$id_diretor = 'Sem Registro';
}
...
<small>
HTML;
if($nivel_usuario == 'Diretor'){
	$query = $pdo->query("SELECT * FROM aprovar_candidatura where diretor = '$id_diretor' ORDER BY id desc");
}else{
...
HTML;
	for($i=0; $i < $total_reg; $i++){
		...
		// $query2 = $pdo->query("SELECT * FROM usuarios where id = '$diretor'");
		// $res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
		// if(@count($res2) > 0){
		// 	$iddiretor = $res2[0]['id_func'];
		// }else{
		...
		$query2 = $pdo->query("SELECT * FROM funcionarios where id = '$diretor'");
		$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
		if(@count($res2) > 0){
			 $nome_diretor = $res2[0]['nome'];
			 $telefone_diretor = $res2[0]['telefone'];
			 $email_diretor = $res2[0]['email'];
		}else{
